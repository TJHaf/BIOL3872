...
#This is a famous data set from R.A. Fisher1 recording functional attributes for three plant species
...
# by column numbers 1 to 2
iris[,1:2]
# by row numbers 1 to 2
iris[1:2,]
...
#What are the exact measurements (Sepal.Length, Sepal.Width, Petal.Length, Petal.Width) of the first and last data entry? Store each answer as a vector named first and last.
...
plot(iris, 
     col = "forestgreen", 
     pch = 19)
#pch means plotting character and specifies the she shape of points 
...
#Investigate help (?) function for 'iris' and 'plot' to visualize the relationships for each species

#Which species has (on average) the longest and widest petals? Store it as a character in your environment.
...
#can look at which species has the longest or widest petals 
#look at facotrd at bottom of output in the console, setosa is factor 1, so it is black on the plot, versicolor is reed, viginica is green

#or 
plot(iris$Species, iris$Petal.Length)
#shows that petal length is longest for virginica 

plot(iris$Species, iris$Petal.Width)
#shows that petal width is longest for virginica
...
str(iris)
#shows that species has 3 factor levels 
plot(iris$Species)
#the three species are setosa, versicolor, virginica
longest_petal_length <- "virginica"

plot(iris$Petal.Width, iris$Species)

widest_petal_length <- "virginica"
...
#filtering the species column by all the virginicas etc

mean(virginica$Petal.Length)
mean(setosa$Petal.Length)
mean(versicolor$Petal.Length)
#taking mean of petal.length column in each subsetted dataframe (by each species), determined longest petal length was held by virginica

Largest_av_petal_length <- "virginica"
:::
...
#Visually (i.e., by inspecting the histogram), what type of distribution appears to describe the sepal width across the whole data set? What is the shape of the distribution for each species?
...
#we subset these rows but we still have an empty virginica column on the figure, this is because Species is a factor with three levels, we would need to edit out the vriginca factor (use str(iris)) to check the structure of the dataset 
...
Here is an incredibly hacky (BAD!) way to do some the same thing without the subset function.
...
<<<<<<< HEAD
# Add legend on the coordinates (1, 7.5)
=======
# Add legend
>>>>>>> refs/remotes/origin/main
legend(1,7.5, 
...
To be precise, the layered grammar defines the components of a plot as:• a default data set and set of mappings from variables to aesthetics,• one or more layers, with each layer having one geometric object, one statistical transformation, one position adjustment, and optionally, one data set and set of aesthetic mappings,• one scale for each aesthetic mapping used,• a coordinate system,• the facet specification.
...
#we want to colour it by each species and make a spoint plot aka scatter plot, legen added automatically

#when you want to colour something by group it NEEDS TO BE INCLUDED IN THE AESTHETIC part of the function aes()
...
#theme can change the background of the graph 

Note that the geom argument can also take an aesthetic, if you want unique color scales for each geom.Also note that the legend inherits the alpha (transparency) argument we include here. Consider how to adjust this in the legend.
...
ggplot(data = iris, aes(x = Petal.Length, y = Petal.Width, color = Species))+
  ...
  # x axis xustomization with labels specified by sequence
<<<<<<< HEAD
  #notice you can add a second aesthetic 
  scale_x_continuous(breaks = seq(0,8,1))

Task 4

Recreate a ggplot comparing sepal lengths and widths. Use any geom you think is appropriate (try a few).

Modify both the x and y axes to show breaks every 0.5 cm.

Create a new iris object (e.g., iris_task4) where the Species factor column is reversed and recreate this plot

{r}
# YOUR CODE HERE
iris %>% 
  mutate(Species = factor(Species, levels = rev(levels(iris$Species)))) %>%  
  ggplot(aes(x= Species, y = Petal.Length, color = Species, size = Sepal.Length))+
 geom_point()

#mutate created a new column called species which is a factor within the OG speceis column with the reverse order of factors, we then piped it into a plot
...
Next week we will dive into refining visualizations with better color palettes and enhanced customization. I recommend that you check out this paper5 and the oce package6, which was developed and is maintained (actively!) by Dr. Dan Kelly (Dalhousie Oceanography). Remember, proficiency is gained by consistent practice!
...
Fisher, R. A. The Use of Multiple Measurements in Taxonomic Problems. Annals of Human Genetics (1936).

MacNeil, M. A. et al. Recovery potential of the world’s coral reef fishes. Nature 520, 341–344 (2015).

Froese, R. et al. Systemic failure of European fisheries management. Science, 826–828 (2025).

Anderson, S. Trade offs working in base R vs. ggplot. Simon Fraser University, (2021).

Thyng, K., et al. True Colors of Oceanography: Guidelines for Effective and Accurate Colormap Selection. Oceanography 29, 9–13 (2016).
