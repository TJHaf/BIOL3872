...
datetime1 <- as.POSIXlt("February-28-2010 2:38PM", format = "%B-%d-%Y %H:%M", tz = "Canada/Pacific")

with_tz(datetime1, tzone = "Canada/Atlantic")
...
datetime2 <- as.POSIXlt("Sep-14-2015 09:50:45", format = "%b-%d-%Y %H:%M:%S", tz = "UTC")
with_tz(datetime2, "Canada/Atlantic")
...
datetime3 <- as.POSIXlt("04-8-2024 16:39", format = "%m-%d-%Y %H:%M", tz = "Canada/Atlantic")
...
# C)
Bedford_ts$month_full <- factor(Bedford_ts$month_full, levels=month.name)

Bedford_ts %>% 
  group_by(Year, month_full) %>%
  summarize(Median_Richness = median(Richness)) %>% 
  ggplot(aes(x = month_full, y = Median_Richness, color = Year))+
  geom_point()
...
C)  to find the time zone for Stockholm (Sweden) and assign this to datetimes in a new object called `combined_datetime_stock`.

D)  Convert these times to the Atlantic Canada timezone (AST) and store named these datetimes in `combined_datetime_NS`.
...
# B)
combined_datetime_stock <- with_tz(combined_datetime, tz = "Europe/Stockholm")

# C)
combined_datetime_NS <- with_tz(combined_datetime, tz = "Canada/Atlantic")
...
diff1 <- dates_format[2] - dates_format[1]
diff2 <- dates_format[3] - dates_format[2]
diff3 <- dates_format[4] - dates_format[3]
diff4 <- dates_format[5] - dates_format[4]
diff5 <-dates_format[6] - dates_format[5]
dfftime_v1 <- c(diff1, diff2, diff3, diff4, diff5)

# C)
#only removing the dates AFTER sep 14, 2015
pre_Einstein <- dates_format[1:5]
...
# YOUR CODE HERE
# A)
#Bedford_ts$weeks <- as.Date(Bedford_t$weeks)
 #Bedford_ts_weekly <- Bedford_ts %>%
  #mutate(weeks = floor_date(Date, unit = "week")) %>%
  #arrange(weeks)


 # Bedford_ts[order(Bedford_ts$week_start_date), ]
#weekly_median = tapply(Bedford_ts$Richness, Bedford_ts$weeks, median)

Bedford_ts$weeks <- week(Bedford_ts$Date)
weeks <- unique(Bedford_ts$weeks)
median = NULL
empty_list <- list()

weekly_median = tapply(Bedford_ts$Richness, Bedford_ts$weeks, mean)

weekly_median <- data.frame(Richness = as.vector(weekly_median), weeks= as.numeric(names(weekly_median)))


# B)

Richness_weekly <- Bedford_ts %>% 
  ggplot(aes(x = weeks, y = Richness, fill = month.name[Date]))+
  geom_point()+
  geom_point(data = weekly_median, aes(x = weeks, y = weekly_median))
