...
#if you don't specify the time zone of the data it will normally be assumed it is UTC

#months in 3 letter abbreviations
month.abb
```
...
#make sure to match the formatting (slashes, dashes between the date and the desired formating)
as.Date("13/02/2012", format = "%d-%m-%Y")
...
#having one dates with slashes and another with dashes causes errors when formatting
as.Date(c("13-02-2012", "13/02/2012"), format = "%d-%m-%Y")

# Also, be careful of misreadss
# no specified formatting
as.Date("13-02-2012")

# Note that this function does not retain datetime information - the time will not be in the output
as.Date("2025-02-01 15:35:20")

#is recommended to use the 24 hour clock
...
```{r}
# these functions preseve the times in the output
...
#formatting previously defined date without retyping whole command
format(ex_datetime, format = "%B-%d-%y %I:%M:%S%p")

#gives a list of time zones options > OlsonNames()
...
# checking the time zone of the ex_tz date
tz(ex_tz)
...
flights <- flights %>% 
  select(year:minute)
#selecting the columns from "year" to "minute" in the dataset
```
...
#y axis is day of the week, x is week of the year, coloured by number of flights
```
...
#we can call these built in vectors (month.abb, letters, etc)
Bedford_ts$Month_name <- month.name[Bedford_ts$Month]

#make a new date object from the broken down components: year, month, day 
Bedford_Date = as.Date(make_datetime(year = Bedford_ts$Year,
                                     month = Bedford_ts$Month,
                                     day = Bedford_ts$Day))
#prove(test) it is equal to Bedford_ts$Date 
all(Bedford_Date == Bedford_ts$Date)
...
```{r}
# Microbial richness
# mutating the month names into a factor that is ordered by calendar year, feed into ggplot
r <- Bedford_ts %>% 
...
#scale says that we want breaks between months and the year labels in the 4 digit format 
