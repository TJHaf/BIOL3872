...
marine_survey$environments <- ifelse (marine_survey$species_count > quantile(marine_survey$species_count, probs = 0.75) & marine_survey$temperature_c > mean(marine_survey$temperature_c), "diverse and hot", "not")
...
diverse_hot_envs <- sapply(marine_survey[ ,numeric_columns], function(x) {
  median(ifelse(marine_survey$environments == "diverse and hot", x, NA), 
      na.rm = TRUE)
})
...
counting_sheep <- numeric(length(sheep))
...
A)  Using the `marine_survey` data frame, use the `lapply` function to calculate the summary statistics of each numeric column and store this as a `list` called `numeric_summaries`.
...
# C) 
numeric_summaries_df <- do.call(bind_rows,numeric_summaries) %>% 
mutate(variables = rownames(numeric_summaries_df))
```
:::
