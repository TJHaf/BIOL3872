...
ggplot(iris, aes(x = Sepal.Length, y = Petal.Length, color = Species)) +
  ...
  theme(
    ...
    panel.grid.minor = element_line(color = "gray40", linewidth = 0.5),
    panel.border = element_rect(color = "black", fill = NA),
    axis.title = element_text(size = 10),
    axis.text = element_text(size = 10, color = "black"),
    legend.background = element_rect(color = "black", linewidth = 1),
    axis.ticks = element_line(linewidth = 0.5, color = "gray40"),
...
data(mtcars)
#replicate from above but with mtcars
...
#within dataframe arguments it is important to use "=" and not arrows
...
#can take a number of colors from the viridis color pallete, that is the same as the number of months

# Order seasonally, not alphabetically (R default)
co2_df %>% 
  mutate(month = factor(month, levels = month.abb)) %>% 
#levels = month.abb means the levels of month are the abbreviated names of the months and in calendar order 
...
co2_df %>% 
  ...
  scale_colour_gradient(low = "blue", 
                       high = "#E74C3C")+
  labs(x = "Year", y = expression("pC0"[2] ~ "(ppm)"))+
#can color the value of the graph based on where it is on the y axis 
...
plot(example,
     ...
     pch = (20),
     cex = 2)
#colorsJet colour palette is very bad 
```
...
#top chunk of output are SEQUENTIAL gradients - good for 0-1 

#middle are QUALITATIVE palttes, nice way to color groups where ordering doesn't matter

#bottom one is the DIVERGENT color pallete- ordering does matter, good for changes over time in data 
```
...
#color palletes good for color defiencies
```
...
#WRONF EXERCISE FOR THIS CLASS, SKIP
